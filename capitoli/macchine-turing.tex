\chapter{Macchine di Turing}\label{cha:macchine-di-turing}
...
\section{Enumerazione delle Macchine di Turing}

L'obiettivo di questa sezione è riuscire a codificare una generica Macchina di Turing deterministica con una stringa binaria, in modo
da potere memorizzare questa stringa sul nastro di una macchina specifica.
Inoltre non siamo interessati a questioni di efficienza, pertanto possiamo utilizzare una codifica semplice da
descrivere, anche se richiede maggiore spazio.

Ogni numero naturale $n$ viene rappresentato con la stringa binaria formata da $n$ \zero seguita da un \uno.
Ad esempio il numero $3$ viene rappresentato come \zero\zero\zero\uno.
In altre parole, viene utilizzata una rappresentazione \emph{unaria} dove il simbolo \uno funge da terminatore della codifica.
