...
\begin{example}
	Sia $G=(V,T,O,E)$, con $V=\{E,I\}$ e $T=\{a,b,\zero,\uno,(,),+,*\}$
	...
	\begin{enumerate}
		...
		\item $I\to I\uno$
	\end{enumerate}
	voglio ottenere $a*(a+b\zero\zero)$
	sostituisco sempre a destra (right most derivation)
	$$E\to E*E\to E*(E)\to E*(E+E)\to E*(E+I)\to E+(E+I\zero)$$
	$$\to R+(I+b\zero\zero)\to E*(a+b\zero\zero)\to I*(a+b\zero\zero)\to a*(a+b\zero\zero)$$
	...
	\begin{center}
		\begin{tabular}{|c|c|c|c|c|}
			...
			6     & b\zero\zero       & E   & 1    & 4                       \\
			\hline
			7     & a+b\zero\zero     & E   & 2    & 5,6                     \\
			\hline
			8     & (a+b\zero\zero)   & E   & 4    & 7                       \\
			\hline
			9     & a*(a+b\zero\zero) & E   & 3    & 5, 8                    \\
			\hline
		\end{tabular}
	\end{center}
\end{example}
...
\begin{example}
	...
	$$L=\{\zero^n \uno^n| n\geq 1\}=\{\zero\uno, \zero\zero\uno\uno, \zero\zero\zero\uno\uno\uno,...\}$$
	con $x^n$ intendo una concatenazione di $n$ volte $x$ (che nel nostro caso sono 0 e 1).\\
	posso scrivere:
	$$\zero^n \uno^n =\zero\zero^{n-1} \uno^{n-1}\uno$$
	il nostro caso base sarà la stringa $\zero\uno$, Poi si ha:
	$G=(V,T,P,S)$, $T=\{\zero,\uno\}$, $V=\{S\}$, il caso base $S\to \zero\uno$  e $S\to \zero S\uno$
	il caso passo è quindi: se $w= \zero^{n-1}\uno^{n-1}\in L$ allora $\zero w\uno\in L$.\\
	Ora voglio dimostare che $\zero\zero\zero\uno\uno\uno\in L$, ovvero $S\to*\, \zero\zero\zero\uno\uno\uno$:\\
	$$S\to\, \zero S\uno \to \zero\zero S\uno\uno\to \zero\zero\zero S\uno\uno\uno$$
\end{example}
\begin{theorem}
...
\begin{example}
	formare una grammatica CFG per il linguaggio:
	$$L=\{\zero^n \uno^n| n\geq 0\}=\{\varepsilon, \zero\uno, \zero\zero\uno\uno, \zero\zero\zero\uno\uno\uno,...\}$$
	stavolta abbiamo anche la stringa vuota. Il caso base stavolta è $S\to\varepsilon| \, \zero S\uno$
\end{example}
\begin{example}
...
\begin{example}
	...
	$$P\to \zero P\zero|\,\uno P\uno|\varepsilon$$
...
\begin{example}
	Si ha:
	$$E\to I|\, E+E|\, E*E|\, (E)$$
	$$I\to a|\,b|\,Ia|\,Ib|\,I\zero|\,I\uno$$
	un albero sintattico per $a*(a+b\zero\zero)$ può essere:
	\begin{center}
...
\end{proof}
\begin{example}
	Mostro l'esistenza di una derivazione sinistra dell'albero sintattico di $a*(a+b\zero\zero)$:
	$$E\to^*_{lm}E*E\to^*_{lm}I*E\to^*_{lm}a*E\to^*_{lm}a*(E)\to^*_{lm}a*(E+E)\to^*_{lm}$$
	$$a*(I+E)\to^*_{lm}a*(a+E)\to^*_{lm}a*(a+I)\to^*_{lm}a+(a+I\zero)\to^*_{lm}a*(a+I\zero\zero)\to^*_{lm}a*(a+b\zero\zero)$$
\end{example}
...
\begin{example}
	...
	\begin{enumerate}
		...
		\item $T\to F|\, T+F$
		\item $F\to I|\, (E)$
		\item $I\to a|\,b|\,Ia|,Ib|\,I\zero|\,I\uno$
	\end{enumerate}
\end{example}
...
\end{itemize}
\begin{example}
	$I\to a|\,b|\,Ia|\,Ib|\,I\zero|\,I\uno$ è una grammatica con le produzioni lineari a sinistra.\\
	Potremmo pensarlo a destra $I\to a|\,b|\,aI|\,bI|\,\zero I|\,\uno I$.\\
	Vediamo esempi di produzione con queste grammatiche:
	\begin{itemize}
		\item con $I\to a|\,b|\,Ia|\,Ib|\,I\zero|\,I\uno$ possiamo derivare $ab\zero\uno b\zero$:
		      $$I\to I\zero\to Ib\zero\to I\uno b\zero\to I\zero\uno b\zero\to Ib\zero\uno b\zero\to ab\zero\uno b\zero$$
		\item con $I\to a|\,b|\,aI|\,bI|\,\zero I|\,\uno I$ invece non riusciamo a generare nulla:
		      $$I\to \zero I\to \zero a$$
	\end{itemize}
	definisco quindi un'altra grammatica (con una nuova categoria sintattica):
	$$I\to aJ|\, bJ$$
	$$J\to a|\,b|\,aJ|\,bJ|\,\zero J|\,\uno J$$
	che però non mi permette di terminare le stringhe con \zero\ e \uno, la modifico ancora otterdendo:
	$$I\to aJ|\, bJ$$
	$$J\to a|\,b|\,aJ|\,bJ|\,\zero J|\,\uno J|\,\zero|\,\uno$$
	e questo è il modo corretto per passare da lineare sinistra a lineare destra
\end{example}
\begin{example}
	Sia $G=(\{S\},\{\zero,\uno\},P,S)$ con $S\to \varepsilon|\,\zero|\,\uno|\,\zero S|\,\uno S$. Si ha quindi:
	$$L(G)=\{\zero,\uno\}^*$$
	si hanno comunque due proposizioni ridondanti, riducendo trovo:
	$$S\to \varepsilon|\,\zero S|\,\uno S$$
	con solo produzioni lineari a destra. Con produzioni lineari a sinistra ottengo:
	$$S\to \varepsilon|\, S\zero|\,S\uno$$
\end{example}
\begin{example}
...
\end{example}
\begin{example}
	A partire da $G=(\{S,T\},\{\zero,\uno\},P,S)$ con:
	$$S\to\varepsilon|\,\zero S|\,\uno T$$
	$$T\to \zero T|\,\uno S$$

	trovo come è fatto $L(G)$:
	$$L(G)=\{w\in\{\zero,\uno\}^*|\, w \mbox{ ha un numero di \uno\ pari}\}$$
\end{example}
\begin{example}
	fornire una grammatica regolare a destra e sinistra per:
	$$L=\{w\in\{\zero,\uno\}^*|\, w \mbox{ ha almeno uno \zero\ o almeno un \uno}\}$$
	Si ah che tutte le stringhe tranne quella vuota ciontengono uno \zero\ o un \uno
	quindi  $G=(\{S\},\{\zero,\uno\},P,S)$:
	\begin{itemize}
		\item \textbf{lineare a destra:}
		      $$S\to \zero|\,\uno|\,\zero S|\,\uno S$$
		\item \textbf{lineare a sinistra:}
		      $$S\to \zero|\,\uno|\,S\zero|\,S\uno$$
	\end{itemize}
\end{example}
...
\begin{example}
	Sia $\Sigma=\{\zero, \uno\}$ e $L_{pal}="stringhe\,\, palindrome\,\, binarie"$.
	Quindi, per esempio, $\zero\uno\uno\zero\in L,\,\, \uno\uno\zero\uno\uno\in L$ ma $\uno\zero\zero\uno\zero\not\in L$. Si ha che $\varepsilon$, la stringa vuota, appartiene a $L$. Diamo una definizione ricorsiva:
	\begin{itemize}
		\item \textbf{base:} $\varepsilon,\, \zero\,\ \uno\in L_{pal}$
		\item \textbf{passo:} se $w$ è palindroma allora $\zero w\zero$ è palindromo e $\uno w\uno$ è palindromo
	\end{itemize}
	...
	$$S\to\varepsilon,\, S\to \zero,\, S\to \uno,\, S\to \zero S\zero,\, S\to \uno S\uno$$
\end{example}


riprendiamo l'esempio sopra:
\begin{example}
	$$G_{pal}=(V=\{S\},\, T=\{\zero, \uno\},\, P,\, S)$$
	con:
	$$P=\{S\to\varepsilon,\, S\to \zero,\, S\to \uno,\, S\to \zero S\zero,\, S\to \uno S\uno\}$$
	Si può ora costruire un algoritmo per creare una stringa palindroma a partire dalla grammatica $G$:
	$$\underbrace{S}_{\mbox{start}}\underbrace{\to}_{\mbox{applico una regola}} \uno S\uno \to \zero\uno S\uno\zero\to \underbrace{\zero\uno\zero\uno\zero}_{\mbox{sostituisco variabile}}$$

	con $S,\, \uno S\uno\,\, e\,\, \zero\uno S\uno\zero$ che sono \textit{forme sentenziali}. Posso così ottenere tutte le possibili stringhe. Esiste anche una forma abbreviata:
	$$S\to \varepsilon|\zero|\uno|\zero S\zero|\uno S\uno$$
...
\begin{example}
	Trovo PDA per il linguaggio delle stringhe binarie  palindrome di lunghezza pari: $L=\{ww^R|w\in\{\zero,\uno\}^*\}$. Con $R$ che indica rovesciato\\
	Si ha la CFG $G=(\{P\},\{\zero,\uno\},Prod,P)$ tale che:
	$$P\to \zero P\zero|\uno P\uno|\varepsilon$$
	...
	PDA $P=(\{q_0,q_1,q_2\},\{\zero,\uno\}, \{o,\uno,z_0\},\delta,q_0,z_0,\{q_2\})$
	ovvero:
	$$\delta(q_0,\zero,z_0)=\{(q_0,\uno z_0)\}$$
	$$\delta(q_0,\uno,z_0)=\{(q_0,\zero z_0)\}$$
	$$\delta(q_0,\zero,\zero)=\{(q_0,\zero\zero)\}$$
	$$\delta(q_0,\zero,\uno)=\{(q_0,\zero\uno)\}$$
	$$\delta(q_0,\uno,\zero)=\{(q_0,\uno\zero)\}$$
	$$\delta(q_0,\uno,\uno)=\{(q_0,\uno\uno)\}$$
	$$\delta(q_0,\varepsilon,z_0)=\{(q_0,z_0)\}$$
	$$\delta(q_0,\varepsilon,\zero)=\{(q_0,\zero)\}$$
	$$\delta(q_0,\varepsilon,\uno)=\{(q_0,\uno)\}$$
	$$\delta(q_1,\zero,\zero)=\{(q_1,\varepsilon)\}$$
	$$\delta(q_2,\uno,\uno)=\{(q_1,\varepsilon)\}$$
	$$\delta(q_2,\varepsilon,z_0)=\{(q_2,\zero z_0)\}$$
	...
	\begin{center}
		\begin{tikzpicture}[shorten >=1pt,node distance=3cm,on grid,auto]
			...
			\path[->]
			(q_0) edge node [align=center] {$\varepsilon,\zero/\zero$\\$\varepsilon,\uno/\uno$\\$\varepsilon,z_0/z_0$} (q_1)
			edge [loop below] node [align=center] {$\zero,z_0/\zero z_0$\\$\uno,z_0/\uno z_0$\\$\zero,\zero/\zero\zero$\\$\zero,\uno/\zero\uno$\\$\uno,\zero/\uno\zero$\\$\uno,\uno/\uno\uno$} ()
			(q_1) edge [loop below] node [align=center] {$\zero,\zero/\varepsilon$\\$\uno,\uno/\varepsilon$} ()
...
\end{itemize}
vediamo un esempio con un PDA che accetta $ww^R|\,w\in\{\zero,\uno\}^*$:
\begin{center}
	\begin{tikzpicture}[shorten >=1pt,node distance=3cm,on grid,auto]
		\node[state, initial] (q_0) {$q_0$};
		\node[state] (q_1) [right=of q_0] {$q_1$};
...
\end{theorem}
Questo dimostra che ogni CFL può essere accettato da un PDA accettante per stack vuoto. Per il teorema visto in
precedenza, posso sempre costruire un altro PDA accettante per stati finale. I PDA accettano tutti e soli i
linguaggi CF. Mostrare che accettano solo linguaggi di tipo 2 è complicato.


\begin{Exercise}[label=5volte]
	Descrivere un automa a pila che riconosca il linguaggio $\{\zero^{i}\uno^{j}\due^{5(i+j)}: i,j\ge 0\}$.
\end{Exercise}


\begin{Answer}
	L'idea intuitiva è che l'automa abbia tre fasi, corrispondenti ai caratteri da leggere: rispettivamente \zero, \uno, \due.
	Nelle prime due fasi, per ogni \zero o \uno letto, si aggiungono $5$ simboli \uno alla pila.
	Nella terza fase, per ogni carattere \due letto, si elimina il simbolo \uno in cima alla pila.
	Infine, quando si è letta tutta la parola in input, bisogna verificare che la pila contenga solo $Z_{0}$.
\end{Answer}

\begin{Exercise}[label=fra-il-doppio]
	Descrivere un automa a pila che riconosca il linguaggio $\{\zero^{i}\uno^{j}: 0\le i\le j\le 2i\}$.
\end{Exercise}

\begin{Answer}
	In questo caso vogliamo controllare che ogni \zero sia bilanciato da $1$ o $2$ \uno.
	Non sappiamo quanti \uno saranno corrispondenti ad uno specifico \zero, ma possiamo sfruttare il fatto che un PDA è non deterministico.
\end{Answer}



\subsection{Automi deterministici a pila}\label{sec:DPDA}
...
\begin{Exercise}
	Costruire un PDA per il linguaggio $L=\{\zero^{n} \uno^{n}:  n\geq 1\}$
\end{Exercise}
...
\begin{Exercise}
	Costruire un PDA per il linguaggio $L=\{\zero^{n} \uno^{n}:  n\ge 0\}$
\end{Exercise}
...
\begin{Exercise}
	Costruire un DPDA per il linguaggio $L=\{w\in\Sigma_{B}\kleene: |w|_{\zero}=|w|_{\uno}\}$.
	In altre parole, $L$ è il linguaggio delle parole su $\Sigma_{B}$ che hanno uguale numero di \zero e di \uno.
\end{Exercise}

\begin{Answer}
	Notiamo subito che $L$ non è libera da prefissi: si considerino ad esempio le parole \zero\uno e \zero\uno\zero\uno.
	Ciò implica che non esiste un automa deterministico a pila che accetta per pila vuota e accetta $L$, ma potrebbe
	esistere un automa con stati accettanti.
...
\begin{Exercise}[label=dpda-ij]
	Descrivere un automa deterministico a pila che riconosca il linguaggio $\{\zero^{i}\uno^{j}: 0\le i\le j\}$.
\end{Exercise}

\begin{Answer}
	Consideriamo due fasi, una in cui leggiamo solo \zero e la seconda in cui leggiamo solo \uno.
	Quando riceviamo il primo \uno, passiamo dal primo al secondo stato e al termine controlliamo che la pila contenga solo $Z_{0}$.
	Questo però non permette di riconoscere la parola vuota: per ovviare a questo inconveniente, introduciamo uno stato
	accettante preliminare che prevede una transizione al primo stato nel momento in cui legge il primo \zero.
\end{Answer}
