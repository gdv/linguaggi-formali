...
Molti generatori di parser moderni supportano PEG, tra cui:
\begin{itemize}
    \item \textbf{LPEG} per Lua,
    \item \textbf{PEG.js} per JavaScript,
    \item \textbf{Python's pegy} e \texttt{parsimmon},
    \item \textbf{Rust's pest},
    \item il tool storico \textbf{Packrat Parser} di Bryan Ford.
\end{itemize}

In conclusione, le PEG offrono un approccio alternativo e spesso più pratico alla specifica di sintassi per linguaggi di programmazione, soprattutto quando si desidera un controllo fine sul comportamento del parser e si vuole evitare l'ambiguità tipica delle CFG.
