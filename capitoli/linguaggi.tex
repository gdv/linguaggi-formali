...
\begin{example}\label{exa:L3}
	L'alfabeto $\Sigma_{B}^{3}$ è formato dalle stringhe \zero\zero\zero, \zero\zero\uno, \zero\uno\zero, \zero\uno\uno, \uno\zero\zero, \uno\zero\uno, \uno\uno\zero, \uno\uno\uno.
\end{example}
...
\begin{example}\label{exa:chiusura-kleene}
	$\Sigma_{B}^{*} = \{\mathtt{\epsilon},\zero,\uno,\zero\zero,\zero\uno,\uno\zero,\uno\zero\zero,\zero\zero\zero,\ldots \}$.
\end{example}
...
\begin{example}\label{exa:0n01}
	Le stringhe che consistono in $n$ \zero seguiti da $n$ \uno:
	$\{\varepsilon,\zero\uno,\zero\zero\uno\uno,\zero\zero\zero\uno\uno\uno,\ldots\}$.
\end{example}

\begin{example}\label{exa:0=1}
	le stringhe con un uguale numero di \zero{} e di \uno{}:
	$\{\varepsilon,\zero\uno,\uno\zero,\zero\zero\uno\uno,\zero\uno\uno\zero,\zero\uno\zero\uno,\uno\zero\zero\uno,\ldots\}$.
\end{example}

\begin{example}\label{exa:numeri-primi-binari}
	le stringhe che codificano un numero primo:
	$\{\uno\zero,\uno\uno,\uno\zero\uno, \uno\uno\uno, \ldots\}$.
\end{example}

\begin{example}\label{exa:numeri-primi-unari}
	le stringhe che sono una codifica unaria di un numero primo, senza usare \zero:
...
\begin{example}\label{exa:0001}
	Consideriamo il linguaggio $L$ su alfabeto $\Sigma_{B}$ delle parola che contengono esattamente un \uno e questo
	...
	$T=\{\zero, \uno\}$, e le produzioni $\mathsf{S}\to \mathsf{Z}\mathsf{U}$, $\mathsf{S}\to \mathsf{U}$,
	$\mathsf{U}\to \uno$, $\mathsf{Z}\to \mathsf{Z}\zero$, and $\mathsf{Z}\to \zero$.
\end{example}

Possiamo notare che la stringa \zero\zero\uno viene generata dalla grammatica $G$ tramite la seguente sequenza di
applicazioni di produzione:

\[\mathsf{S} \sprod{\mathsf{S}\to \mathsf{Z}\mathsf{U}} \mathsf{Z}\mathsf{U}  \sprod{\mathsf{Z}\to \mathsf{Z}\zero} \mathsf{Z}\zero\mathsf{U} \sprod{\mathsf{U}\to \uno} \mathsf{Z}\zero\uno \sprod{\mathsf{Z}\to \zero} \zero\zero\uno \]

Questa derivazione non è l'unica possibile per generare la stringa \zero\zero\uno e la grammatica $G$ che abbiamo
...
Un linguaggio può essere visto in tre modi:
\begin{enumerate}
	\item come un insieme di parole;
	\item tramite una grammatica che genera tutte e sole le parole del linguaggio;
	\item tramite una macchina che riconosce tutte e sole le parole del linguaggio.
