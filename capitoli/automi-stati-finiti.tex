\section{Automi a stati finiti}\label{sec:automi-stati-finiti}
...
\begin{example}
	specifico DFA che accetta tutte le strighe binarie in cui compare la sequenza \zero\uno:
	$$L=\{w|w \mbox{ è della forma x\zero\uno y, con x e y pari a \zero{} o \uno{}} \}=\{\zero\uno,\uno\uno\zero\uno,\uno\zero\zero\zero\uno\uno,...\}$$
	o anche:
	$$L=\{x\zero\uno y| x,y\in\{\zero,\uno\}^* \}$$
	abbiamo quindi:
	$$\Sigma=\{\zero,\uno\}$$
	ragioniamo sul fatto che $A$:
	\begin{enumerate}
		\item se ha "già visto" \zero\uno, accetterà qualsiasi input
		\item pur non avendo ancora visto \zero\uno, l'input più recente è stato \zero{}, cosicché se ora vede un \uno{} avrà visto \zero\uno
		\item non ha ancora visto \zero\uno, ma l'input più recente è nullo (siamo all'inizio), in tal caso A non accetta finché non vede
		      uno \zero{} e subito dopo un \uno{}
	\end{enumerate}
	la terza condizione rappresenta lo stato iniziale. All'inizio bisogna vedere uno \zero{} e poi un \uno{}. Ma se nello stato $q_0$ si vede per primo un \uno{} allora non abbiamo fatto alcun passo verso \zero\uno, e dunque dobbiamo permanere nello stato $q_0$, $\delta(q_0,\uno)=q_0$. D'altra parte se nello stato iniziale vedo \zero{} siamo nella seconda condizione, uso quindi $q_2$ per questa condizione, si avrà quindi $\delta(q_0,\zero)=q_2$. Vedo ora le transizoni di $q_2$, se vedo \zero{} ho che \zero{} è l'ultimo simbolo incontrato quindi uso nuovamente $q_2$, $\delta(q_2,\zero)=q_2$, in attesa di un \uno{}. Se arriva \uno{} passo allo stato accertante $q_1$ corrispondente alla prima condizione, $\delta(q_2,\uno)=q_1$. Ora abbiamo incontrato \zero\uno quindi può succedere qualsiasi cosa e dopo qualsiasi cosa accada potremo nuovamente aspettarci qualsiasi cosa, ovvero $\delta(q_1,\zero)=\delta(q_1,\uno)=q_1$. Si deduce quindi che:
	$$Q=\{q_0,q_1,q_2\} \mbox{ e } F=\{q_1\}$$
	quindi:
	$$A=\{\{q_0,q_1,q_2\} ,\{\zero,\uno\}, \delta, q_0, \{q_1\} \}$$
	...
	\begin{center}
		\begin{tikzpicture}[shorten >=1pt,node distance=2cm,on grid,auto]
			...
			\path[->]
			(q_0) edge  node {\zero} (q_2)
			edge [loop below] node {\uno} ()
			(q_2) edge  node  {\uno} (q_1)
			edge [loop below] node {\zero} ()
			(q_1) edge [loop below] node {\zero,\uno} ();
		\end{tikzpicture}
...
\end{example}
\begin{example}
	Trovo automa per: $$L=\{w\in\{\zero,\uno\}^*| w= \zero^n\uno^m\}$$
	vediamo i vari casi:
		...
		\item $n,m\geq 0$:
		      \begin{center}
			      \begin{tikzpicture}[shorten >=1pt,node distance=2cm,on grid,auto]
				      ...
				      \path[->]
				      (q_0) edge  node {\uno} (q_1)
				      edge [loop below] node {\zero} ()
				      (q_1) edge  node  {} (q_E)
				      edge [loop below] node {\uno} ()
				      (q_E) edge [loop below] node {\zero,\uno} ();
			      \end{tikzpicture}
		      \end{center}
		      ovvero posso non generare nulla e uscire subito con $q_0$, generare solo un \uno{} e passare a $q_1$ e uscire oppure generare \zero{} e \uno{} a piacere con l'ultimo stato o generare \zero{} a piacere dal primo e \uno{} a piacere dal secondo.
		\item $n\geq 0 \,\,m>0$:
		      \begin{center}
			      \begin{tikzpicture}[shorten >=1pt,node distance=2cm,on grid,auto]
				      \node[state,initial] (q_0)   {$q_0$};
			      ...
			      \end{tikzpicture}
		      \end{center}
		      ovvero come l'esempio sopra solo che non posso uscire in $q_0$ in quanto almeno un \uno{} deve essere per forza generato
		\item $n> 0\,\, m\geq 0$:
		      \begin{center}
			      \begin{tikzpicture}[shorten >=1pt,node distance=2cm,on grid,auto]
				      \node[state,initial] (q_0)   {$q_0$};
				      ...
				      \path[->]
				      (q_0) edge  node {\zero} (q_1)
				      edge [bend right] node {\uno} (q_E)
				      (q_1) edge  node {\uno} (q_2)
				      edge [loop above] node {\zero} ()
				      (q_2) edge node {\zero} (q_E)
...
\end{example}
\begin{example}
	Trovo automa per: $$L=\{x\zero\uno\zero y|\,x,y\in\{o,1\}^*\}$$
	\begin{center}
		\begin{tikzpicture}[shorten >=1pt,node distance=2cm,on grid,auto]
			\node[state, initial] (q_0) {$q_{0}$};
			\node[state] (q_1) [right=of q_0] {$q_{1}$};
			...
			(q_1) edge  node {\uno} (q_2)
			edge [loop above] node {\zero} ()
			(q_2) edge  node {\zero} (q_3)
			(q_3) edge [loop below] node {\zero,\uno} ();
		\end{tikzpicture}
	\end{center}
\end{example}
...
\begin{example}
	Sia $L=\{x\zero\uno|\,x\in\{o,1\}$ ovvero il linguaggio formato da tutte le stringhe binarie che terminano in \zero\uno.\\
	...
	\begin{center}
		\begin{tikzpicture}[shorten >=1pt,node distance=2cm,on grid,auto]
			...
			(q_1) edge  node {\uno} (q_2)
			edge [loop above] node {\zero} ()
			(q_2) edge  [bend left =25] node {\zero} (q_1)
			edge [bend left = 50] node {\uno} (q_0);
		\end{tikzpicture}
	\end{center}
	...
	\begin{center}
		\begin{tikzpicture}[shorten >=1pt,node distance=2cm,on grid,auto]
			...
			\path[->]
			(q_0) edge  node {\zero} (q_1)
			edge [loop above] node {\zero,\uno} ()
			(q_1) edge  node {\uno} (q_2);
		\end{tikzpicture}
	\end{center}
	quindi con:
	$$\delta(q_0,\zero)=\{q_0,q_1\}$$
	$$\delta(q_0,\uno)=\{q_0\}$$
	$$\delta(q_1,\zero)=\emptyset$$
	$$\delta(q_1,\uno)=\{q_2\}$$
	$$\delta(q_2,\zero)=\emptyset$$
	$$\delta(q_2,\uno)=\emptyset$$
	in forma tabulare:
	\begin{center}
		\begin{tabular}{c|c|c}
			$\delta$    & \zero         & \uno        \\
		...
		\end{tabular}
	\end{center}
	vediamone la simulazione per la stringa \zero\zero\uno\zero\uno:
	\begin{center}
		\includegraphics[scale=0.7]{img/nfa.png}
	\end{center}
	ovvero si parte dallo stato inziale, quando viene letto \zero{} si passa a $q_0$ e $q_1$, poi viene letto il secondo \zero{} quindi $q_0$ va nuovamente verso $q_0$ e $q_1$ mentre il primo $q_1$ muore non avendo transizioni su \zero{}. Arriva poi l'\uno{} quindi $q_0$ va solon verso $q_0$ e $q_1$ verso $q_2$ e sarebbe accettante ma l'input non è finito. Ora arriva \zero{} e $q_2$ si blocca mentre $q_0$ va sia in $q_0$ che in $q_1$. Arriva infine un \uno{} che manda $q_0$ in $q_0$ e $q_1$ in $q_2$ che è accettante e non avendo altri input si è dimostrata l'appartenenza della stringa al linguaggio
\end{example}
...
\begin{example}
	Automa per $L=\{x\zero\uno\zero y|\,x,y\in\{\zero,\uno\}^*\}$ ovvero tutte le stringhe con dentro la sequenza $\zero\uno\zero$:
	\begin{center}
		\begin{tikzpicture}[shorten >=1pt,node distance=2cm,on grid,auto]
			\node[state, initial] (q_0) {$q_{0}$};
			\node[state] (q_1) [right=of q_0] {$q_{1}$};
			...
			(q_1) edge  node {\uno} (q_2)
			(q_2) edge  node {\zero} (q_3)
			(q_3)     edge [loop above] node {\zero,\uno} ();
		\end{tikzpicture}
	\end{center}
\end{example}
...
\begin{center}
	\begin{tikzpicture}[shorten >=1pt,node distance=4cm,on grid,auto]
		...
		edge [loop above] node {\zero} ()
		(q_2) edge [bend left =25]  node {\zero} (q_1)
		edge [bend left =45] node  {\uno} (q_0);
	\end{tikzpicture}
\end{center}
...
\begin{itemize}
	...
	\item $\forall S\subseteq Q_N$ e $ \forall a \in\Sigma$:
	      ...
	      per esempio:
	      $$\delta_D(\{q_0,q_1,q_2\},\zero)=\delta_N(q_0,\zero)\cup \delta_N(q_1,\zero) \cup\delta_N(q_2,\zero) $$
\end{itemize}
...
\begin{example}
	automa DFA per $w=x\zero\uno\zero y,\,x,y\in\{\zero,\uno\}^*$ :\\
	la stringa più corta è \zero\uno\zero
	\begin{center}
		\begin{tikzpicture}[shorten >=1pt,node distance=2cm,on grid,auto]
			\node[state, initial] (q_0) {$q_{0}$};
			\node[state] (q_1) [right=of q_0] {$q_{1}$};
...
\newpage
\begin{example}
	cerco DFA per  stringhe di bit non contegano \zero\zero\zero
	\begin{center}
		\begin{tikzpicture}[shorten >=1pt,node distance=2cm,on grid,auto]
			\node[state, initial, accepting] (q_0) {$q_0$};
			\node[state, accepting] (q_1) [right=of q_0]{$q_1$};
			...
			\node[state] (q_e) [right=of q_2]{$q_E$};
			\path[->]
			(q_0) edge  [bend left=25] node {\zero} (q_1)
			edge [loop] node {\uno} ()
			(q_1) edge node {\zero} (q_2)
			edge [bend left=25] node {\uno} (q_0)
			(q_2) edge node {\zero} (q_e)
			edge  [bend left=55] node{\uno} (q_0)
			(q_e) edge [loop ] node {\zero,\uno} ();
		\end{tikzpicture}
	\end{center}
\end{example}
\begin{example}
	cerco DFA per  stringhe di bit non contegano \zero\zero\zero almeno una volta
	\begin{center}
		\begin{tikzpicture}[shorten >=1pt,node distance=2cm,on grid,auto]
			\node[state, initial] (q_0) {$q_0$};
			\node[state] (q_1) [right=of q_0]{$q_1$};
...
\begin{example}
	...
	\begin{center}
		\begin{tikzpicture}[shorten >=1pt,node distance=2cm,on grid,auto]
			...
			(q_2) edge node {\zero} (q_3)
			edge  [bend left=45] node{\uno} (q_0)
			(q_3) edge node {\zero} (q_e)
			edge node {\uno} (q_4)
			(q_4) edge [bend left=25] node {\zero} (q_5)
			edge [loop right] node {\uno} ()
			(q_5) edge [bend left=25] node {\uno} (q_4)
			edge node {\zero} (q_6)
			(q_6) edge [bend right=25] node {\zero} (q_e)
			edge [bend left=55] node {\zero} (q_4)
			(q_e) edge [loop ] node {\zero,\uno} ();
		\end{tikzpicture}
	\end{center}
\end{example}
...
\begin{center}
	\begin{tikzpicture}[shorten >=1pt,node distance=2cm,on grid,auto]
		...
		(q_0) edge  node {\uno} (q_1)
		edge [loop above] node {\zero,\uno} ()
		(q_1) edge  node {\zero,\uno} (q_2)
		(q_2) edge node {\zero,\uno} (q_3)
		(q_3) edge node {\zero,\uno} (q_4);
	\end{tikzpicture}
\end{center}
che definisce un $L\subseteq\{\zero,\uno\}^*$ tale che $w\in L$ sse n-simo elemento della fine è 1. Si ha:
$$|Q_N|=n+1\to |Q_D|=2^n$$
...
\begin{example}
	creo E-NFA per
	$$ER=(\zero+\uno)^*1(\zero+\uno)$$
	si ha che per \zero+\uno:
	\begin{center}
		\begin{tikzpicture}[shorten >=1pt,node distance=2cm,on grid,auto]
			\node[state,initial] (q_0)   {$q_0$};
			\node[state] (q_1) [above right =of q_0] {$q_1$};
			...
			(q_0) edge node {$\varepsilon$} (q_1)
			(q_0) edge node {$\varepsilon$} (q_2)
			(q_1) edge node {$\zero$} (q_3)
			(q_2) edge node {$\uno$} (q_4)
			(q_3) edge node {$\varepsilon$} (q_5)
			(q_4) edge node {$\varepsilon$} (q_5);
		\end{tikzpicture}
	\end{center}
	da qui ottengo $(\zero+\uno)^*$
	\begin{center}
		\begin{tikzpicture}[shorten >=1pt,node distance=2cm,on grid,auto]
...
\begin{example}
	...
	$$R_{A,C}^{'}=\emptyset+\uno\emptyset^*(\zero+\uno)=\uno(\zero+\uno)$$
	ovvero:
	\begin{center}
		\begin{tikzpicture}[shorten >=1pt,node distance=3cm,on grid,auto]
			\node[state, initial] (q_0) {$A$};
	...
	$$R_{A,D}=\emptyset+1(\zero+\uno)\emptyset^*(\zero+\uno)=\uno(\zero+\uno)(\zero+\uno)$$
	che è:
	\begin{center}
		\begin{tikzpicture}[shorten >=1pt,node distance=4cm,on grid,auto]
			\node[state, initial] (q_0) {$A$};
			...
			\path[->]
			(q_0) edge  node {1(\zero+\uno)(\zero+\uno)} (q_3)
			edge [loop above] node {\zero+\uno} ();
		\end{tikzpicture}
	\end{center}
	quindi:
	$$E_1=((\zero+\uno)+1(\zero+\uno)(\zero+\uno)\emptyset^*\emptyset)^*1(\zero+\uno)(\zero+\uno)\emptyset^*=(\zero+\uno)^*1(\zero+\uno)(\zero+\uno)$$
	ottengo $E2$ eliminando D che non ha successori, quindi:
	$$E_2=(\zero+\uno)^*1(\zero+\uno)$$
	e quindi infine:
	$$E=E_1+E_2=(\zero+\uno)^*1(\zero+\uno)(\zero+\uno)+(\zero+\uno)^*1(\zero+\uno)=(\zero+\uno)^*1(\zero+\uno)(\varepsilon+\zero+\uno)$$
\end{example}
\begin{example}
	\begin{center}
		\begin{tikzpicture}[shorten >=1pt,node distance=3cm,on grid,auto]
			...
			(q_0) edge  node {\zero} (q_1)
			edge [loop above] node {\uno} ()
			(q_1) edge [bend left = 25] node {\zero} (q_3)
			(q_1) edge  node {\uno} (q_2)
			(q_2) edge  [bend left = 25] node {\uno} (q_3)
			edge [loop above] node {\zero} ()
			(q_3) edge  [bend left = 25] node {\uno} (q_1)
			(q_3) edge  [bend left = 25] node {\zero} (q_0);
		\end{tikzpicture}
	\end{center}
	rimuovo $r$:
	$$s\to r:\uno$$
	...
	\begin{center}
		\begin{tikzpicture}[shorten >=1pt,node distance=3cm,on grid,auto]
			...
			(q_0) edge  node {\zero} (q_1)
			edge [loop above] node {\uno} ()
			(q_1) edge [bend left = 25] node {$\zero+\uno\zero^*\uno$} (q_3)
			(q_3) edge  [bend left = 25] node {\uno} (q_1)
			(q_3) edge  [bend left = 25] node {\zero} (q_0);
		\end{tikzpicture}
	\end{center}
	...
	$$p\to s:\zero$$
	$$q\to s:\uno$$
	$$s\to q:\zero+\uno\zero^*\uno$$
	non ha loop e non ha archi diretti tra $p$ e $q$ ($R_{p,q}=\emptyset$). Si ottengono quindi:
	$$R_{p,q}=\emptyset+\zero\emptyset^*(\zero+\uno\zero^*\uno)=\zero(\zero+\uno\zero^*\uno)$$
	$$R_{q,q}=\emptyset+\uno\zero^*(\zero+\uno\zero^*\uno)=\uno(\zero+\uno\zero^*\uno)$$
	\begin{center}
		\begin{tikzpicture}[shorten >=1pt,node distance=4cm,on grid,auto]
			\node[state, initial, accepting] (q_0) {$p$};
			\node[state] (q_3) [right=of q_0] {$q$};
			\path[->]
			(q_0) edge [bend left = 25] node {$\zero(\zero+\uno\zero^*\uno)$} (q_3)
			edge [loop above] node {\uno} ()
			(q_3) edge  [bend left = 25] node {\zero} (q_0)
			edge [loop above] node {$\uno(\zero+\uno\zero^*\uno)$} ();
		\end{tikzpicture}
	\end{center}
	elimino ora $q$ che ha $p$ sia come predecessore che come successore:
	$$p\to q: \zero(\zero+\uno\zero^*\uno)$$
	$$q\to p:\zero$$
	$$loop: \uno(\zero+\uno\zero^*\uno)$$
	$$R_{p,p}:\uno$$
	ottenendo:
	$$R^{'}_{p,p}=\uno+\zero(\zero+\uno\zero^*\uno)(\uno(\zero+\uno\zero^*\uno))^*\zero=\uno+(\zero\zero+\zero\uno\zero^*\uno)(\uno\zero+\uno\uno\zero^*\uno)^*\zero$$
	\newpage
	ovvero:
	...
	\begin{center}
		\begin{tikzpicture}[shorten >=1pt,node distance=4cm,on grid,auto]
			\node[state, initial, accepting] (q_0) {$p$};
			\path[->]
			(q_0)   edge [loop above] node {$\uno+(\zero\zero+\zero\uno\zero^*\uno)(\uno\zero+\uno\uno\zero^*\uno)^*\zero$} ();
		\end{tikzpicture}
	\end{center}
	Essendo $p$ sia iniziale che finale si ha:
	$$E=(\uno+(\zero\zero+\zero\uno\zero^*\uno)(\uno\zero+\uno\uno\zero^*\uno)^*\zero)^*$$
\end{example}
...
\begin{example}
	...
	\begin{center}
		\begin{tikzpicture}[shorten >=1pt,node distance=2cm,on grid,auto]
			...
			(q_0) edge node {\zero} (q_1)
			edge [loop above] node {\uno} ()
			(q_1) edge [loop above] node {\zero,\uno} ();
		\end{tikzpicture}
	\end{center}
	...
	\begin{center}
		\begin{tikzpicture}[shorten >=1pt,node distance=2cm,on grid,auto]
			...
			(q_1) edge  node {\zero} (q_3)
			edge [loop above] node {\uno} ()
			(q_2) edge  node {\uno} (q_3)
			edge [loop below] node {\zero} ()
			(q_3) edge [loop below] node {\zero,\uno} ();
		\end{tikzpicture}
	\end{center}
	$$\delta((p,r),\zero)=(\delta_L(p,o),\delta_M8r,o))=(q,r)$$
\end{example}
...
\begin{example}
	...
	\begin{center}
		\begin{tikzpicture}[shorten >=1pt,node distance=2cm,on grid,auto]
			...
			\node[state] (q_1) [right=of q_0] {$b$};
			\path[->]
			(q_0) edge [bend left = 25] node {\uno} (q_1)
			edge [loop above] node {\zero} ()
			(q_1) edge [bend left = 25] node {\zero} (q_0)
			edge [loop above] node {\uno} ();
		\end{tikzpicture}
	\end{center}
	e $B$:
	\begin{center}
		\begin{tikzpicture}[shorten >=1pt,node distance=3cm,on grid,auto]
			...
			\path[->]
			(q_0) edge  node {\zero} (q_1)
			edge [bend left =25] node {\uno} (q_2)
			(q_1) edge node {\uno} (q_2)
			edge [loop above] node {\zero} ()
			(q_2) edge [bend left =25] node {\zero} (q_0)
			edge [loop below] node {\uno} ();
		\end{tikzpicture}
	\end{center}
	faccio la tabella:
	...
	\begin{center}
		\begin{tikzpicture}[shorten >=1pt,node distance=3cm,on grid,auto]
			...
			\path[->]
			(q_0) edge  node {\zero} (q_2)
			edge [bend left =25] node {\uno} (q_1)
			edge [loop above] node {\zero} ()
			(q_1) edge [bend left =25] node {\zero} (q_0)
			edge [loop above] node {\uno} ()
			(q_2) edge  node {\uno} (q_1);
...
\begin{example}
	...
	\begin{center}
		\begin{tikzpicture}[shorten >=1pt,node distance=3cm,on grid,auto]
			...
			(q_0) edge [bend left =25] node {\zero} (q_1)
			edge [bend left =25] node {\uno} (q_2)
			(q_1) edge [bend left =25] node {\uno} (q_2)
			edge [loop above] node {\zero} ()
			(q_2) edge [bend left =25] node {\zero} (q_0)
			edge [bend left =25] node {\uno} (q_1);
		\end{tikzpicture}
...
\begin{example}
	...
	\begin{center}
		\begin{tikzpicture}[shorten >=1pt,node distance=4cm,on grid,auto]
			...
			\path[->]
			(q_0) edge node {\uno} (q_5)
			edge node {\zero} (q_1)
			(q_1) edge node {\uno} (q_2)
			edge node [below] {\zero} (q_6)
			(q_2) edge [bend right =25] node {\zero} (q_0)
			edge [loop above] node {\uno} ()
			(q_3) edge node {\zero} (q_2)
			edge node [below]{1} (q_6)
			(q_4) edge node {\uno} (q_5)
			edge [bend right =45] node {\uno} (q_7)
			(q_5) edge node [above] {\zero} (q_2)
			edge node {\uno} (q_6)
			(q_6) edge [bend left =25] node {\uno} (q_4)
			edge [loop below] node {\zero} ()
			(q_7) edge node {\zero} (q_6)
	...
	\begin{itemize}
		...
		\item \zero{} non li distingue perché li porta rispettivamente in \textit{b} e \textit{g}, che non sono accettanti
		\item \uno{} non li distingue perché li porta rispettivamente in \textit{f} e \textit{e}, che non sono accettanti
		\item \zero\uno{} li distingue perché $\stackrel{\wedge}{\delta}(a,\zero\uno)=c$ e $\stackrel{\wedge}{\delta}(g,\zero\uno)=e$ e il primo è accettante mentre il secondo no, quindi i due stati non sono equivalenti
	\end{itemize}
...
\begin{center}
	...
	\end{tabular}
\end{center}
poiché, per esempio $c,h$ è distinguibile e gli stati $e$ e $f$ con \zero{} vanno in $h$ e $c$ si ha che anche $e$ e $f$ sono distinguibili, segno quindi nelle caselle appropriate. Proseguo controllando anche le altre coppie e riempio le caselle. Ottengo:
\begin{center}
	\begin{tabular}{c|c|c|c|c|c|c|c|}
		\hline
		b & x & / & / & / & / & / & / \\
...
\\$\delta$ è $\delta_L$ per $Q_L$ e $\delta$ è $\delta_M$ per $Q_M$
	...
	\begin{example}
		...
		\begin{center}
			\begin{tikzpicture}[shorten >=1pt,node distance=3cm,on grid,auto]
				...
				\path[->]
				(q_0) edge node {\zero} (q_1)
				edge [bend right=25] node {\uno} (q_2)
				(q_1) edge node {\uno} (q_2)
				edge [loop above] node {\zero} ()
				(q_2) edge [bend right =25] node {\zero} (q_0)
				edge [loop below] node {\uno} ();
			\end{tikzpicture}
		\end{center}
		entrambi i linguaggi accettano $(\varepsilon+(\zero+\uno)^*\zero)$
		applico la tabella:
		\begin{center}
	...
	\begin{example}
		...
		\begin{center}
			\begin{tikzpicture}[shorten >=1pt,node distance=4cm,on grid,auto]
				...
				\path[->]
				(q_0) edge node {\uno} (q_4)
				edge node {\zero} (q_1)
				(q_1) edge node {\zero} (q_2)
				edge [bend left =25] node {\uno} (q_5)
				(q_2) edge [bend left =25] node {\uno} (q_7)
				edge node {\zero} (q_3)
				(q_3) edge node {\uno} (q_7)
				edge [bend right= 85] node {\zero} (q_4)
				(q_4) edge node {\zero} (q_5)
				edge [bend left =25] node {\uno} (q_8)
				(q_5) edge [bend left = 25] node {\zero} (q_1)
				edge node {\zero} (q_6)
				(q_6) edge node {\uno} (q_1)
				edge node {\zero} (q_7)
				(q_7) edge node {\zero} (q_8)
				edge [bend left = 25] node {\uno} (q_2)
				(q_8) edge [bend left = 25] node {\uno} (q_4)
				edge [bend left = 85] node {\zero} (q_0);
			\end{tikzpicture}
		\end{center}
		faccio la tabella
		\begin{center}
		...
		\begin{center}
			\begin{tikzpicture}[shorten >=1pt,node distance=4cm,on grid,auto]
				...
				\path[->]
				(q_0) edge node {\zero,\uno} (q_1)
				(q_1) edge [bend left =25] node {\zero,\uno} (q_2)
				(q_2) edge [bend left =25] node {\uno} (q_1)
				edge node {\zero} (q_0);
			\end{tikzpicture}
		\end{center}
	\end{example}
	...
	$$L_{01}=\{\zero^n\uno^n|\,n\geq 1\}=\{\zero\uno,\zero\zero\uno\uno,...\}$$
	supponiamo che questo linguaggio sia rappresentabile da un DFA $A$ con $k$ stati ($L(A)=L_{01}$).\\
$\zero^k$ implica $k+1$ prefissi: $\varepsilon,\zero,\zero\zero,\zero\zero\zero,...,\zero^k$ e quindi $\exists i,j$ tali che $\zero^i$ e $\zero^j$ finiscono nello stesso stato. Allora l'automa è ingannabile e accetterebbe $\zero^i\zero^j$ con $i\neq j$.\\
Formalmente si ha:
\begin{theorem}
...
\end{proof}
\begin{example}
	mostriamo che L01 non è regolare. Suppongo per assurdo che sia regolare, allora vale il pumping lemma. Sia $n\in\mathbb{N}$ la costante del pumping lemma. Pongo $w=\zero^n\uno^n=xyz$ tale che $|xy|=n$, ovvero $|xy|=\zero^n$, quindi $xy$ è formato da soli \zero. Poniamo $x=\zero^{n-1}\,\,y=\zero\neq \varepsilon\,\, z=\uno^n$. Però per il pumping lemma $\forall k\geq 0\,\,\ xy^kz\in L_{01}$.
	$$xy^kz=\zero^{n-1}\zero^k\uno^n=\zero^{n+k-1}\uno^n\in L_{01}$$
	ma se $k\neq 1$ ciò non è vero e quindi il linguaggio non è regolare
\end{example}
\begin{example}
	$L=\{w\in\{\zero,\uno\}^*|\, w=w^R\}$, linguaggio delle stringhe palindrome, è regolare?\\
	suppongo per assurdo di sì, con $n$ costante del pumping lemma.\\
	$w=\zero^n\uno\zero^n$ quindi $x=\zero^{n-1}\,\,y=\zero\,\,z=\uno\zero^n$.
	Per $k=0$ si ha $xy^kz=\zero^{n-1}\uno\zero^n\not\in L$, quindi non è regolare
\end{example}
\begin{example}
	$L=\{\uno^p|\, p\,\,primo\}=\{\uno\uno,\uno\uno\uno,\uno\uno\uno\uno\uno\}$ è regolare?\\
	$w=\uno^p=\uno^{n-1}\uno\uno^{p-n}$ con:
	$$x=\uno^{n-1}\,\,y=\uno\,\,z=\uno^{p-n}$$
	per $k=0$ si ha $\uno^{p-1}$ e $p-1$ non è , quindi non è regolare
\end{example}
\begin{example}
	$L=\{\zero^n\uno^m|\, n\leq m\}$ è regolare?\\
	$w=\zero^{n-l}\zero^l\uno^{m}$ con:
	$$x=\zero^{n-l}\,\,y=\zero^l\,\,z=\uno^{m}$$
	con $|y|=l$ e $0<l\leq n$\\
	quindi $\forall k\geq 0$ si ha $xy^kz\in L=\zero^{n-l}\zero^{kl}\uno^{m}$ con:
	$$x=\zero^{n-l}\,\,y=\zero^{kl}\,\,z=\uno^{m}$$
	scelgo $k$ tale che $b-l+kl=n+(k-1)l>m\to k> 1+\frac{m-n}{l}$ quindi non è regolare
\end{example}
\begin{example}
	$L=\{\zero^n\uno^m|\, n\geq m\}$ è regolare?\\
	$w=\zero^{n-l}\zero^l\uno^{m}$ con:
	$$x=\zero^{n-l}\,\,y=\zero^l\,\,z=\uno^{m}$$
