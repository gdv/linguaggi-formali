...
I parser depth-first ricostruiscono una visita in profondità dell'albero di derivazione.
...
regola di produzione: corretta.
Consideriamo adesso la regola di produzione: da sinistra verso destra: per ogni terminale controlliamo che sia identico al
corrispondente carattere della string in input.
Se ciò non è vero, dobbiamo rifiutare la regola di produzione e passare ad un'altra produzione.
...
\begin{enumerate}
\item la porzione in input elaborata;
...
\item la predizione\lepo{il termine predizione è usato con un significato diverso da quello usato precedentemente}, ovvero la porzione di forma sentenziale ancora da confermare.
\end{enumerate}
...
I parser depth-first non possono gestire grammatice ricorsive a sinistra o produzioni che hanno come membro destro
...
$A$ e una forma sentenziale $\alpha$.
Talvolta si distingue fra ricorsione diretta a sinistra se esiste una passo di derivazione $A\sprod{} A\alpha$ e ricorsione
indiretta a sinistra se sono necessarie almeno due produzioni per ottenere una forma sentenziale $A\alpha$.\lepo{Dire
  perchè si fa questa trasformazione}

Iniziamo con l'eliminazione di tutte le $\varepsilon$-produzioni nella forma $A\to \varepsilon$\lepo{Non cambia il
  linguaggio generato?} e siano
$A\to \alpha_{1} | \cdots | \alpha_{n}$ tutte le altre produzioni che hanno $A$ come membro sinistro.
Sostituiamo tutte le produzioni che hanno $A$ come testa  con

\begin{gather}
A\to \alpha_{1} | \cdots | \alpha_{n},
\end{gather}

mentre ogni produzione nella forma $B\to \alpha A\beta$ dove $A$ compare all'interno del corpo viene sostituita
con la produzione
\begin{gather}
B\to \alpha A_{1}\beta | \alpha \beta
\end{gather}\lepo{e $A_{1}$?}

Le nuove produzioni non cambiano il linguaggio generato e rendono esplicite le derivazioni che possono
generare $\varepsilon$: infatti quello che possiamo dimostrare è che la lunghezza di tutte le derivazioni in zero o più passi
...
caso, una volta che la derivazione corrispondente a $A\eprod{} A\alpha$ viene generata, ci troveremmo a
...
Un insieme di produzioni che porta alle stesse forme sentenziali è:\lepo{serve un esempio}
...
Per rimuovere anche la ricorsione indiretta a sinistra dobbiamo ordinare le variabili, ottenendo
$\langle A_{1}, \ldots, A_{n}\rangle$ e garantire che non ci siano produzioni $A_{i} \to A_{j}\alpha$ con $j<i$.
...
È obbligatorio procedere a queste sostituzioni dopo avere eliminato tutte le produzioni ricorsive dirette a sinistra per
...
In particolare viene realizzato un parser a discesa ricorsiva specifico per la grammatica che si vuole considerare,
...
L'idea fondamentale è considerare la regola $S\to  \uno B | \zero A$ come una procedura che ha successo se (1) trova un
\uno e la procedura $B$ ha successo, oppure (2) trova un \zero e la procedura $A$ ha successo.

\begin{algorithm}[bt!]
\SetKwInOut{Input}{Input}\SetKwInOut{Output}{Output}
\If{trova \uno e ProceduraB()}{\Return vero}
\If{trova \zero e ProceduraA()}{\Return vero}
\Return falso
