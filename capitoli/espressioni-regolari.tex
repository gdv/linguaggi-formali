...
\begin{proposition}\label{prop:regex-associative-commutative}
	Valgono le seguenti proprietà delle operazioni su espressioni regolari:
	\begin{itemize}
		\item
...
\begin{enumerate}
	...
	\item $[\widehat{\,} abc]$, dove $a$, $b$, $c$ sono tutti simboli di $\Sigma$, corrisponde alla scelta fra i simboli \emph{non} indicati.
...
\begin{example}\label{exa:01}
	Si consideri il linguaggio su $\Sigma_{B}$ delle parole non nulle formate da  \zero e \uno alternati --- equivalentemente, nessuna
	parola contiene la sottostringa  \zero\zero e \uno\uno ---
	Trovare un'espressione regolare che ha associato tale linguaggio.

	Dividiamo le parole del linguaggio a seconda che abbiano lunghezza pari o dispari e che inizino per \zero e
	\uno, facendo l'unione di questi quattro linguaggi.
	Otteniamo così l'espressione $(\zero\uno)^* | (\uno\zero)^* | \uno(\zero\uno)^* | \zero(\uno\zero)^*$.

	\[\zero\uno\to \{\zero\uno\}\]
	$$(\zero\uno)^*\to \{\epsilon, \zero\uno, \zero\uno\zero\uno,\zero\uno\zero\uno\zero\uno,\cdots\}$$
	$$(\zero\uno)^{*} + (\uno\zero)^{*} \to \{ \epsilon, \zero\uno,\uno\zero,\zero\uno\zero\uno,\uno\zero\uno\zero, \cdots \} $$
	ma posso volere diverse quantità di \zero e \uno, sempre mantenendo l'alternanza, metto o uno \zero o un \uno davanti a quanto ottenuto appena sopra:
	$$(\zero\uno)^*+(\uno\zero)^*+\zero(\uno\zero)^*+\uno(\zero\uno)^*\to \{\epsilon,\zero\uno,\uno\zero,\zero\uno\zero,\uno\zero\uno,\cdots\}$$
	non è comunque l'unica soluzione, si può avere:
	$$(\epsilon+\uno)(\zero\uno)^*(\epsilon+\zero)\to \{\epsilon,\zero\uno,\uno\zero,\zero\uno\zero,\uno\zero\uno,\cdots\}$$
	oppure ancora:
	$$(\epsilon+\zero)(\uno\zero)^*(\epsilon+\uno)$$
\end{example}


\begin{Exercise}[label=3-1]
	Ho $E=(\zero+\uno)^*\zero^*(\zero\uno)^*$:
	\begin{itemize}
		\item \zero\zero\uno fa parte del linguaggio?
		\item \uno\zero\zero\uno fa parte del linguaggio?
		\item \zero\uno\zero\uno fa parte del linguaggio?
		\item \zero fa parte del linguaggio?
		\item \uno\zero fa parte del linguaggio?
	\end{itemize}
\end{Exercise}
...
\begin{Answer}
	\begin{itemize}
		\item \zero\zero\uno è una parola del linguaggio: $\epsilon\cdot \zero\cdot \zero\uno$
		\item \uno\zero\zero\uno è una parola del linguaggio: $\uno\cdot \zero\cdot \zero\uno$
		\item \zero\uno\zero\uno è una parola del linguaggio: $\epsilon\cdot\epsilon \cdot \zero\uno\zero\uno$
		\item \zero è una parola del linguaggio: $\epsilon\cdot \zero\cdot \epsilon$
		\item \uno\zero è una parola del linguaggio: $\uno\cdot \zero\cdot \epsilon$
	\end{itemize}



	Notare che in realtà $L((\zero+\uno)^*)= \Sigma_{B}^*$ e le parti che seguono sono opzionali (la chiusura di Kleene permette di
...
\begin{example}
	ho $ER=((\zero\uno)^*\cdot \uno\zero\cdot (\zero+\uno)^*)^*$
	\begin{itemize}
		\item \zero\uno\zero\uno fa parte del linguaggio? No
		\item \zero\uno\zero\zero\zero fa parte del linguaggio? No
		\item \zero\uno\zero\uno\uno fa parte del linguaggio? No
		\item \uno\zero\uno\uno\uno fa parte del linguaggio? Si, $\epsilon\cdot \uno\zero\cdot \uno\uno\uno$
		\item \uno\zero\uno\zero\uno\zero fa parte del linguaggio? Si, prendo $\uno\zero\cdot \uno\zero\uno\zero$
		\item \uno\zero\uno\uno\zero\uno fa parte del linguaggio? Si, $\epsilon\cdot \uno\zero\cdot \uno$ due volte
		\item \zero\uno\zero\uno\uno\zero\zero\zero\uno\uno fa parte del linguaggio? Si, $\zero\uno\zero\uno\cdot \uno\zero\cdot \zero\zero\uno\uno$ (\zero\zero\uno\uno lo posso prendere da $(\zero+\uno)^*$)
	\end{itemize}
\end{example}
\begin{example}
	ho $ER=((\zero\uno)^*\cdot \uno\zero\cdot (\zero+\uno))^*$
	\begin{itemize}
		...
		\item \zero\uno\zero\zero\zero fa parte del linguaggio? No
		\item \zero\uno\zero\uno\uno fa parte del linguaggio? No
		\item \uno\zero\uno\uno\uno fa parte del linguaggio? No
		\item \uno\zero\uno\zero\uno\zero fa parte del linguaggio? No
		\item \uno\zero\uno\uno\zero\uno fa parte del linguaggio? Si, $\epsilon\cdot \uno\zero\cdot \uno$ due volte
		\item \zero\uno\zero\uno\uno\zero\zero\zero\uno\uno fa parte del linguaggio? No
	\end{itemize}
\end{example}
\begin{example}
	Da $L\subseteq\{\zero,\uno\}|\mbox{ stringhe contenenti almeno una volta \zero\uno}$
	quindi:
	$(\zero+\uno)^*\zero\uno(\zero+\uno)^*$
\end{example}
\begin{example}
	ho $ER=(\zero\zero^*\uno^*)^*$, quindi:
	$L=\{\epsilon,\zero,\zero\uno,\zero\zero\zero,\zero\zero\uno,\zero\uno\zero,\zero\uno\uno\}=\{\epsilon\}\cup\{ w\in \{\zero,\uno\}^* |\text{ w che inizia con \zero}\}$
\end{example}
...
\begin{example}
	ho $ER=(\zero^*\uno^*)^*\zero\zero\zero(\zero+\uno)^*$, quindi, sapendo che $\{\zero,\uno\}^*$ mi permette tutte le combinazioni che voglio come $(\zero+\uno)^*$:
	$L=\{w\in\{\zero,\uno\}^*|\text{ w come voglio con tre \zero consecutivi}\}$
\end{example}
\begin{example}
...
\end{example}
\begin{example}
	Da $L\subseteq\{\zero,\uno\}$
	ogni \uno è seguito da \zero, a meno che non sia l'ultimo carattere, ovvero \uno\uno non compare

	quindi:
	$(\uno\zero+\zero)^*(\epsilon+\uno)^*$
\end{example}
\begin{example}
	cerco ER per $L\subseteq\{\zero,\uno\}^*|\, \text{stringhe contenenti un numero pari di \uno}$:
	$(\zero^*\uno\zero^*\uno)^*\zero^*$
	oppure:
	$(\zero+\uno\zero^*\uno)^*$
\end{example}
\begin{example}
	cerco ER per $L\subseteq\{\zero,\uno\}^*|\, \text{stringhe contenenti un numero dispari di \uno}$:
	$(\zero^*\uno\zero^*)^*\zero^*\uno\zero^*$
	oppure:
	$(\zero+\uno\zero^*\uno)^*\uno\zero^*$
\end{example}
\begin{example}
	cerco ER per $L\subseteq\{\zero,\uno\}^*|\, \text{stringhe contenenti un numero divisibile per 3 di \zero}$:
	$(\uno^*\zero\uno^*\zero\uno^*\zero)^*\uno^*$
\end{example}
\begin{example}
	cerco ER per $L\subseteq\{\zero,\uno\}^*|\, \text{stringhe contenenti al più una coppia di \uno consecutivi}$:
	$(\uno\zero+\zero)^*(\uno\uno+\uno+\epsilon)(\zero\uno+\zero)^*$
\end{example}
\begin{example}
...
\noindent\begin{minipage}{\textwidth}
	\includegraphics[width=\textwidth]{diagramma-transizione}
	\captionof{figure}{Esempio di diagramma di transizione}
	\label{fig:diagramma-transizione}
\end{minipage}
...
$\delta(q_1, c) = q_2 $.
\begin{example}
	Consideriamo il linguaggio $L$ formato dalle strighe binarie che hanno la sottostringa \zero\uno.
	Equivalentemente $L=\{ x\zero\uno y: x,y \in \Sigma_{B}^{*}\}$.
	...
	\begin{enumerate}
		...
		\item pur non avendo ancora visto 01, l'input più recente è stato 0.
		      Se leggerà \uno, andrà nello stato accettante, altrimenti rimane nello stesso stato;
		\item non ha ancora visto \zero, e rimane in questo stato finchè non legge \zero.
	\end{enumerate}
...
\noindent\begin{minipage}{\textwidth}
	\includegraphics[width=\textwidth]{dfa-sottostringa-01}
	\captionof{figure}{DFA che accetta il linguaggio $\Sigma_{B}^{*}\zero\uno\Sigma_{B}^{*}$}
	\label{fig:dfa-sottostringa-01}
\end{minipage}
...
\begin{Exercise}\label{exe:101-dfa}
	Costruire un DFA per il linguaggio $L\in\Sigma_{B}$ delle parole che contengono la sottostringa \uno\zero\uno.
\end{Exercise}


\begin{Exercise}\label{exe:no-101-dfa}
	Costruire un DFA che riconosca il linguaggio $L\in\Sigma_{B}$ delle parole che non contengono la sottostringa \uno\zero\uno.
\end{Exercise}

\begin{Answer}
	\includegraphics[width=\textwidth]{dfa-no-sottostringa-101}
\end{Answer}
...
\begin{Exercise}\label{exe:no-101-er}
	Costruire un'espressione regolare per il linguaggio $L\in\Sigma_{B}$ delle parole che non contengono la sottostringa \uno\zero\uno.
\end{Exercise}
...
\begin{Answer}
	...
	Una definizione equivalente è l'insieme delle parole in cui ogni occorrenza del carattere \uno è seguita da un
	altro \uno, oppure da \zero\zero, oppure ancora tale \uno è uno degli ultimi due caratteri della parola (in
	questo caso, non potrà essere il carattere iniziale di una sottostringa \uno\zero\uno)
	Questa formulazione si presta maggiormente a disegnare un'espressione regolare.
	Infatti, una riscrittura diretta di quanto descritto sopra, tenendo presente che un numero arbitrari di \zero
	potrebbe precedere il primo \uno e seguire ogni \zero, diventa
	$$\zero^{*}(\uno\zero\zero\zero^{*})^{*} (\uno\zero |\uno | \epsilon)$$ che può essere ulteriormente
	semplificata in
	$$\zero^{*}(\uno\zero\zero\zero^{*})^{*} \uno^{*} \zero^{*}.$$
\end{Answer}
...
\begin{Exercise}\label{exe:no-diff2-er}
	...
	\end{enumerate}
	Sono quindi parole del linguaggio $\epsilon$, \zero\uno, \uno\zero, \uno\zero\uno\zero\uno\zero, \zero\uno\zero\uno.
	Non sono parole del linguaggio \uno\uno\zero\zero.
\end{Exercise}
...
\section{Da espressioni regolari a DFA}
\label{sec:da-DFA-a-ER}

In questa sezione descriveremo una procedura che, partendo da un'esprssione regolare $R$, costruisce un $\epsilon$-NFA \nfa che
riconosce esattamente il linguaggio rappresentato da $R$.
La costruzione segue la definizione di espressione regolare.
Di conseguenza analizziamo prima i casi base: quelli in cui $E$ non contiene nessuna operazione.

\begin{enumerate}
	\item se $R=\epsilon$, allora $L(R)=\{\epsilon\}$ e l'automa \nfa ha due stati: quello iniziale $q_{0}$ e
	      quello accettante $q_{1}$.
	      Inoltre esiste una $\epsilon$-transizione da $q_{0}$ a $q_{1}$.
	      In altre parole, $\delta(q_{0},\epsilon) = q_{1}$.
	      \begin{center}
		      \begin{tikzpicture}[shorten >=1pt,node distance=2cm,on grid,auto]
			      \node[state,initial] (q_0)   {$q_0$};
			      \node[state, accepting] (q_1) [right=of q_0] {$q_1$};
		      ...
		      \end{tikzpicture}
	      \end{center}
	\item se $R=\emptyset$, allora $L(R)=\emptyset$ e l'automa \nfa ha due stati: quello iniziale $q_{0}$ e quello accettante $q_{1}$.
	      Non ci sono transizioni.
	      In altre parole, $\delta(q_{0},\cdot)$ non è mai definita.
	      \begin{center}
		      \begin{tikzpicture}[shorten >=1pt,node distance=2cm,on grid,auto]
			      \node[state,initial] (q_0)   {$q_0$};
			      \node[state, accepting] (q_1) [right=of q_0] {$q_1$}; ;
		      \end{tikzpicture}
	      \end{center}
	\item se $R=a$, dove $a$ è un carattere, allora  $L(R)=\{a\}$ e l'automa \nfa ha due stati: quello iniziale $q_{0}$ e
	      quello accettante $q_{1}$.
	      Inoltre esiste una transizione etichettata  $a$ da $q_{0}$ a $q_{1}$.
	      In altre parole, $\delta(q_{0},a) = q_{1}$.
	      \begin{center}
		      \begin{tikzpicture}[shorten >=1pt,node distance=2cm,on grid,auto]
			      \node[state,initial] (q_0)   {$q_0$};
			      \node[state, accepting] (q_1) [right=of q_0] {$q_1$};
...
In tutti i casi base, è immediato notare che il linguaggio accettato dall'automa corrisponde sempre a $L(R)$.
Passiamo adesso a considerare cosa accade quando l'espressione regolare $R$ corrisponda ad applicare un'operazione a due
espressioni regolari $S$ e $T$.
Per ipotesi induttiva, assumiamo di avere già costruito un automa
$\mathcal A_{S} = \langle \Sigma, Q_{S}, q_{0,S}, F_{s}, \delta_{S}$ che riconosce il linguaggio $L(S)$ rappresentato da
$S$ e un automa
$\mathcal A_{T} = \langle \Sigma, Q_{T}, q_{0,T}, F_{T}, \delta_{T}$ che riconosce il linguaggio $L(S)$ rappresentato da
$S$.
Inoltre costruiremo sempre un automa con un solo stato finale.
Sempre per ipotesi induttiva possiamo denotare con $q_{f,S}$ e  $q_{f,T}$ rispettivamente lo stato finale di
$\mathcal A_{S}$ e  $\mathcal A_{T}$.
In tutti i casi andremo a costruire un automa \nfa che accetta esattamente $L(R)$, partendo dall'automa che è l'unione
di $\mathcal_{S}$ e  $\mathcal_{T}$, ovvero $Q$ è l'unione disgiunta di $Q_{S}$ e $Q_{T}$ e, per ogni stato $q\in Q$ e
ogni carattere $c\in \Sigma \cup \{epsilon\}$, $\delta(q,c) = \delta_{S}(q,c)$ se $q\in Q_{S}$ e  $\delta(q,c) = \delta_{T}(q,c)$ se $q\in Q_{T}$.


Se $R=S+T$, allora $L(R)=L(S)+L(T)$.
Intuitivamente, dobbiamo scegliere se la parola in input appartiene a $L(S)$ oppure a $L(T)$.
Per realizzare questa scelta, lo stato iniziale $q_{0}$ di $\mathcal A$ ha due $\epsilon$-transizioni: una verso
$q_{0,S}$ e una verso $q_{0,T}$.
Inoltre aggiungiamo a $Q$ un nuovo stato $q_{R}$ che sarà l'unico stato finale, e due  $\epsilon$-transizioni: una da
$q_{S}$ a $q_{R}$ e una da $q_{0,T}$ a  a $q_{R}$.
Notiamo che l'automa descritto accetta il linguaggio $L(S) \cup L(T)$ e ha un solo stato finale.

\begin{center}
	\begin{tikzpicture}[shorten >=1pt,node distance=2cm,on grid,auto]
		\node[state,initial] (q_0)   {$q_0$};
		\node[state] (q_1) [above right =of q_0] {$q_{0,S}$};
		\node[state] (q_2) [below right =of q_0] {$q_{0,T}$};
		\node[draw=none,fill=none] (Q_E) [right = of q_1] {$S$};
		\node[draw=none,fill=none] (Q_F) [right = of q_2] {$T$};
		\node[state] (q_3) [right=of Q_E] {$q_S$};
		\node[state] (q_4) [right=of Q_F] {$q_T#$};
		\node[state, accepting] (q_5) [below right =of q_3] {$q_R$};
		\path[->]
		(q_0) edge node {$\epsilon$} (q_1)
...
\label{sec:pumping-lemma-RE}

Il pumping lemma (e lemma dell'espansione) esplicita una proprietà dei linguaggi regolari: ogni parola sufficientemente
lunga può essere espansa, ovvero esiste una sua sottostringa che può essere ripetuta un numero arbitrario di volte,
ottenendo sempre una nuova parola del linguaggio.

\begin{lemma}[Lemma dell'espansione]\label{lem:pumping-lemma}
	Sia $L$ un linguaggio regolare.
	Allora esiste una costante $p$, detta costante di espansione, tale che per ogni parola $w\in L$ con $|w|\le p$, la parola $w$ può
	essere decomposta come $w=xyz$ con $|xy|\le p$, $y\ne \epsilon$, e $xy^{i}z$ è una parola del linguaggio $L$ per ogni intero $i\ge 0$.

	Notiamo immediatamente che questo lemma non permette di trarre conclusioni se il linguaggio $L$ non
	è regolare.
	In particolare, anche se $L$ non è regolare, potremmo avere una decomposizione $xyz$ di una parola tale che $xy^{i}z$
	è sempre una parola di $L$.
\end{lemma}

\begin{proof}
	Siccome $L$ è un linguaggio regolare, allora esiste un \dfa che accetta esattamente $L$.
	Sia $p$ il numero di stati $|Q|$ e sia $w$ una parola del linguaggio $L$ con lunghezza maggiore di $p$.
	Sia $r_{0}, \ldots , r_{|w|}$ la sequenza di stati di $A$ attraversati quando viene letto l'input $w$, ovvero
	$r_{i} = \hat{\delta}(q_{0}, w[:i])$.
	Siccome $|w|>p$, allora esistono $i,j$ tali che $r_{i}=r_{j}$ e $i<j$.
	Allora definiamo $x$, $y$, $z$ rispettivamente come la sottostringa di $w$ che precede l'ingresso in $r_{i}$, che va da
	$r_{i}$ a $r_{j}$, che segue $r_{j}$ --- formalmente $x= w[:i]$, $y= w[i+1:j]$,  $z= w[j+1:]$.
	È immediato notare che $xyz=w$ e $y\ne \epsilon$, perchè $j>i$.
	Dalla definizione \gdv{TODO} di transizione estesa, $\hat{\delta}(q_{0}, w) = \hat{\delta}(\hat{\delta}(\hat{\delta}(q_{0}, w[:i]), w[i+1:j]), w[j+1:])$.
	Siccome $r_{i}=r_{j}$,  $\hat{\delta}(r_{i}, w[i+1:j]) = r_{i}$,
	$\hat{\delta}(q_{0}, xy^{i}z) = \hat{\delta}(\hat{\delta}(\hat{\delta}(q_{0}, w[:i]), w[i+1:j]), w[j+1:])=\hat{\delta}(\hat{\delta}(q_{0}, w[:i]),  w[j+1:])$
	per ogni intero $i\ge 0$.
	Siccome $w\in L$, allora $\hat{\delta}(q_{0}, xyz)\in F$: ciò implica che $\hat{\delta}(q_{0}, xy^{i}z)\in F$ per ogni intero $i>0$,
...
Una particolarità del lemma dell'espansione è che utilizziamo la contronominale,\sidenote{La contronominale di
	$p\Rightarrow q$ è $\lnot q \Rightarrow \lnot p$} quindi mostreremo che non vale la tesi
del \Cref{lem:pumping-lemma} e, come conseguenza, avremo dimostrato che il linguaggio $L$ non è regolare.
Riscriviamo quindi il \Cref{lem:pumping-lemma} come la sua contronominale.

\begin{lemma}[Lemma dell'espansione --- contronominale]\label{lem:pumping-lemma-2}
	Sia $L$ un linguaggio.
	Se per ogni costante $p$ esiste una parola $w\in L$ con $|w|\le p$, tale che per ogni decomposizione di $w$ come $xyz$, con
	$|xy|\le p$, $y\ne \epsilon$, esiste un intero $i\ge 0$ tale che $xy^{i}z\notin L$, allora $L$ non è regolare.
\end{lemma}
...
\begin{Exercise}\label{exe:pump-01multiplo}
	Dato il linguaggio $L\in\Sigma_{B}\kleene$ delle parole nella forma $\uno^{l}\zero^{k}$, tale che $l$ è un multiplo di $k$,
	determinare se è
	regolare.
	In  caso affermativo, dare un'espressione regolare equivalente a $L$, altrimenti dimostrare che non è regolare.
\end{Exercise}
...
\begin{Exercise}\label{exe:pump-01entrodoppio}
...
Si prende la stringa $\zero^{n!}\uno\zero^{(n+1)!}$, l'espansione generica con $|y| = k$ è  $\zero^{n! + (i-1)k}\uno\zero^{(n+1)!}$.
