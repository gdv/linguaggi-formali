...
% 								$$L^*=L^0=\{\varepsilon\}$$
% 					\item $L=\{\varepsilon\}$ implica $L^0=\{\varepsilon\}=L=L^1=L^2=\cdots$, si ha quindi:
% 								$$L^*=\{\varepsilon\}=L$$
% 				\end{itemize}
...
quindi:
$$\forall a\in \Sigma \mbox{ e } \forall S=\{p_1,\cdots,p_k\}, \forall S\in Q_F \mbox{ e } Q_E$$
$$\delta_F(S,a) \mbox{ si ottiene }\cup \delta_E(p_i,a)=\{r_1,\cdots, r_n\}$$
...
$0^k$ implica $k+1$ prefissi: $\varepsilon,0,00,000,\cdots,0^k$ e quindi $\exists i,j$ tali che $0^i$ e $0^j$ finiscono nello stesso stato. Allora l'automa è ingannabile e accetterebbe $0^i0^j$ con $i\neq j$.\\
	Formalmente si ha:
	\begin{theorem}
	...
	\end{theorem}
	\begin{proof}
		essendo $L\in REG$ esiste un DFA $A$ tale che $L=L(A)$. Suppongo $|Q|=n$ e considero $w=a_1a_2\cdotsa_n$ con $m\geq n$. Sia:
		$$p_i=\stackrel{\wedge}{\delta}(q_0,a_1,\cdotsa_i)\,\,\forall i=0,1,\cdots,n$$
		quindi:
		$$p_0=q_0,p_1,\cdots,p_n\to n+a\,\,stati$$
		allora $\exists i,j$, con $0\leq i<j\leq n$ tali che $p_i=p_j$. Scompongo ora $w$ in $w=xyz$ con:
		$$x=a_1a_2\cdotsa_n$$
		$$y=a_{i+1}a_{i+2}\cdotsa_j$$
		$$z=a_{j+1}a_{j+2}\cdotsa_m$$
		\begin{center}
			\begin{tikzpicture}[shorten >=1pt,node distance=3cm,on grid,auto]
				\node[state, initial] (q_0) {$p_0$};
